      ...
      >
        <div className="container mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link href="/#hero" className="text-2xl font-bold bg-[#0253EE] bg-clip-text text-transparent">
              &lt;A/&gt;
            </Link>
            ...
            <div className="lg:hidden">
              <button onClick={() => setIsMenuOpen((s) => !s)} className="p-2 text-[#0253EE] flex items-center justify-end" aria-label={isMenuOpen ? 'Close menu' : 'Open menu'}>
                <span className="flex flex-col gap-1 items-end justify-center" aria-hidden="true" style={{ lineHeight: 0 }}>
                  <motion.span className="block w-9 h-1 bg-current rounded-full origin-center" variants={topBarVariants} animate={isMenuOpen ? 'open' : 'closed'} transition={{ type: 'tween', duration: 0.18 }} />
                  <motion.span className="block w-5 h-1 bg-current rounded-full origin-center" variants={middleBarVariants} animate={isMenuOpen ? 'open' : 'closed'} transition={{ type: 'tween', duration: 0.18 }} />
                  <motion.span className="block w-7 h-1 bg-current rounded-full origin-center" variants={bottomBarVariants} animate={isMenuOpen ? 'open' : 'closed'} transition={{ type: 'tween', duration: 0.18 }} />
                </span>
              </button>
            </div>
          </div>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="container mx-auto px-6 h-full flex flex-col">
              <div className="flex items-center justify-between h-20">
                <Link href="/#hero" onClick={() => setIsMenuOpen(false)} className="text-2xl font-bold bg-[#0253EE] bg-clip-text text-transparent">
                  &lt;A/&gt;
                </Link>
              ...
              <div className="flex-grow flex flex-col items-center justify-center gap-6 text-center">
                ...
                <div className="w-full border-t border-black mt-8" aria-hidden="true" />
